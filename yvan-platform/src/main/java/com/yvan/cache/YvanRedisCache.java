...
public class YvanRedisCache extends RedisCache {

    private Logger LOGGER = LoggerFactory.getLogger(YvanRedisCache.class);
     ...
     * @param expiration
     */
    public YvanRedisCache(String name, byte[] prefix, RedisOperations<? extends Object, ? extends Object> redisOperations,
                      long expiration) {
        ...
        if(exists.booleanValue()){
            try {
                if(LOGGER.isDebugEnabled()){
                    LOGGER.debug("from " + new String(cacheKey.getKeyBytes()));
                }
            }catch (Exception ex){
                LOGGER.error("from redis cache log error",ex);
            }
        }
    ...
    public void put(RedisCacheElement element) {
        ...
        try {
            if(LOGGER.isDebugEnabled()){
                LOGGER.debug("put "+new String(element.getKeyBytes()));
            }
        }catch (Exception ex){
            LOGGER.error("put redis cache log error",ex);
        }
        redisOperations
    ...
    public ValueWrapper putIfAbsent(RedisCacheElement element) {
        ...
        try {
            if(LOGGER.isDebugEnabled()){
                LOGGER.debug("putIfAbsent "+new String(element.getKeyBytes()));
            }
        }catch (Exception ex){
            LOGGER.error("putIfAbsent redis cache log error",ex);
        }
    ...
    public void evict(final RedisCacheElement element) {
        ...
        try {
            if(LOGGER.isDebugEnabled()){
                LOGGER.debug("evict "+new String(element.getKeyBytes()));
            }
        }catch (Exception ex){
            LOGGER.error("evict redis cache log error",ex);
        }
